...
- **定义**：库是如何将 TypeScript 代码转换为 GraphQL Schema 的。
  ...
  - **依赖复杂度**：引入该库所需的外部依赖数量及其体积。优秀的架构模式应当尽量减少依赖，实现安装即用。差的架构模式往往依赖许多魔法（如装饰器需要 `reflect-metadata`、`class-validator` 等），且需要额外的构建过程（如代码生成、编译时插件）和复杂的安装步骤（如配置 TypeScript 编译选项、启用实验性装饰器支持）。
- **评判标准**：优秀的架构模式应当在提供足够灵活性的同时，最小化运行时开销，并保持代码的纯净与可测试性。避免过度的元数据反射（Reflection）和全局副作用是现代库的趋势。构建过程应当简单、配置集中，支持模块化构建以便于大型项目的拆分与协作。**依赖越少越好**：轻量级的库通常意味着更快的安装速度、更小的 Bundle 体积以及更低的维护成本。理想的架构模式应当实现**安装即用**，无需额外的构建步骤、代码生成或复杂的配置。避免强制绑定特定的运行时框架（如必须使用特定版本的 Class Validator 或 Reflect Metadata）是加分项。
...
- **定义**：库如何定义和组织 Resolver（解析器）逻辑，以及如何处理 GraphQL 的输入参数（Arguments）和输入对象（Input Object）的定义、类型推导以及验证。
  - **解析器定义**：如何定义 Query、Mutation 和 Field Resolver。支持高内聚的模块化组织，利于领域驱动开发（DDD），能够将业务逻辑与 Schema 定义优雅地解耦或紧密集成。
...
- **评判标准**：代码应当简洁且具备高度的类型安全性。参数定义应当具备完整的类型推导能力，能提供更好的 IDE 提示。验证逻辑应当易于复用和组合，显著提升代码的可维护性。理想的库应当能够将验证逻辑与 Schema 定义完全合一，极大地减少 Resolver 内部的校验代码。
