...
| 维度                | 得分 (1-5) | 简评                                              |
| :------------------ | :--------- | :------------------------------------------------ |
| **1. 架构模式**     | <-待评分-> | 极简依赖、纯运行时构建、零魔法、完全中立          |
| **2. 类型定义**     | <-待评分-> | 逻辑关联、显式注册、需要手动维护同步              |
| **3. 解析器与验证** | <-待评分-> | 参数类型可推断，但无模块化、无验证、无 DataLoader |
| **4. 内置功能**     | <-待评分-> | 核心功能完善，但高级功能缺失                      |
| **5. 生态集成**     | <-待评分-> | GraphQL Server 兼容性佳，但 ORM 和验证库集成缺失  |
...
| 评估项         | 得分       | 说明                                   |
| :------------- | :--------- | :------------------------------------- |
| **依赖复杂度** | <-待评分-> | 仅依赖 `graphql`，零运行时开销         |
| **构建流程**   | <-待评分-> | 纯运行时构建，无需代码生成             |
| **配置魔法**   | <-待评分-> | 零魔法，完全符合原生 TS 实践           |
| **生态集成**   | <-待评分-> | 完全中立，可与任何 GraphQL Server 集成 |
...
| 评估项         | 得分       | 说明                                                       |
| :------------- | :--------- | :--------------------------------------------------------- |
| **单一数据源** | <-待评分-> | TypeScript 类型与 GraphQL 类型逻辑关联，但需要手动维护同步 |
| **枚举支持**   | <-待评分-> | 需要显式注册，不支持自动推断                               |
| **接口继承**   | <-待评分-> | 支持接口，但需要手动重复声明字段                           |
| **联合类型**   | <-待评分-> | 支持 Union，但需要手动实现 resolveType                     |
| **类型推断**   | <-待评分-> | 参数类型可推断，字段类型需要显式声明                       |
