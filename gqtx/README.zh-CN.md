...
**内置功能评分依据**：
- Directives：✅ 内置支持（5分）- 原生支持，通过 `directives` 参数传递
- Extensions：✅ 内置支持（5分）- 原生支持，可在 Field 和 ObjectType 上定义扩展
- DataLoader：⛔ 无法实现（0分）- 无内置支持，需要手动实现
- Scalars：✅ 内置支持（5分）- 通过 `Gql.Scalar()` 定义，API 直观
- Subscription：✅ 内置支持（5分）- 原生支持，通过 async generator 实现
- Context：✅ 内置支持（5分）- 原生支持，通过模块扩展实现全局类型
- Middleware：⛔ 无法实现（0分）- 完全不支持中间件机制
- Query Complexity：⛔ 无法实现（0分）- 完全不支持查询复杂度分析
- Depth Limiting：⛔ 无法实现（0分）- 完全不支持深度限制

**内置功能总分：25/45 = 2.8/5.0**
