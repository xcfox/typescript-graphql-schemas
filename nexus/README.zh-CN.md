...
1. **极简配置架构**：采用 Builder 模式 + 代码生成，不依赖装饰器、反射元数据或非标准 TS 语法。虽然代码生成技术上可选，但实际使用中几乎必须使用以获得完整类型安全。
...
Nexus 是一个采用 Builder 模式 + 代码生成的 GraphQL Schema 构建库，通过极简配置的方式提供了强大的类型安全和模块化设计。虽然代码生成需要额外配置和维护，且在某些方面（如验证、DataLoader、ORM 集成）需要手动实现，但其完全中立的架构和强大的类型安全使其成为需要避免装饰器和反射元数据的项目的不错选择。
